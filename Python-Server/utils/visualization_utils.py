...
def draw_bounding_box_on_image_array(current_frame_number, image,
    ...
    """
    image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
    is_vehicle_detected, csv_line, update_csv, predicted_speed = draw_bounding_box_on_image(current_frame_number,
                                                                                            image_pil, ymin, xmin,
                                                                                            ymax, xmax, color,
                                                                                            thickness, display_str_list,
                                                                                            use_normalized_coordinates)
    np.copyto(image, np.array(image_pil))
    return is_vehicle_detected, csv_line, update_csv, predicted_speed
...
def visualize_boxes_and_labels_on_image_array(current_frame_number, image,
    ...
    for i in range(min(max_boxes_to_draw, boxes.shape[0])):
        # print('scores ',scores[i],' ',min_score_thresh)
    ...
    for box, color in box_to_color_map.items():
        ...
        if (("car" in display_str_list[0]) or ("truck" in display_str_list[0]) or ("bus" in display_str_list[0])):
            is_vehicle_detected, csv_line, update_csv, predicted_speed = draw_bounding_box_on_image_array(
                current_frame_number,
                image,
                ymin,
                xmin,
    ...
    return counter, csv_line_util, predicted_speed
